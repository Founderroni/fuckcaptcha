...
from fuckcaptcha.exceptions.hard_challenge_exception import HardChallengeException
...
class Solver:
    ...
    def get_challenge(self, token, region):
        ...
        return self.client.post(
            ...
            headers={
                ...
                "sec-ch-ua-platform": '"Linux"',
                "sec-ch-ua": '"Chromium";v="104", " Not A;Brand";v="99", "Google Chrome";v="104"',
                "referer": "https://client-api.arkoselabs.com/fc/gc/?token="
                + token.replace("|", "&"),
                "sec-fetch-dest": "empty",
                "sec-fetch-mode": "cors",
                "sec-fetch-site": "same-origin",
    ...
    def answer_normal(
        self, api_breaker, region, session_token, challenge_token, input_answer
    ):
    ...
    def answer_rotate(self, answer, challenge, challenge_token, region, session_token):
        ...
        new_answer = (
            round(answer * increment)
            if 0.0 <= answer <= round(360 / 51.4) - 1
            else answer
        )
    ...
    def solve(self):
        ...
        if not self.load_captcha(session_token, region)["logged"]:
            return {"solved": False, "token": original_token}
        challenge = self.get_challenge(session_token, region)
        challenge_id = challenge["challengeID"]
        waves = challenge["game_data"]["waves"]
        if waves > 2:
            raise HardChallengeException(
                "Challenge too hard of. Not able of completer. Sir. Ofing. Ok"
            )
        if challenge["game_data"]["gameType"] == 3:
            api_breaker = challenge["game_data"]["customGUI"]["api_breaker"]
            for _ in range(waves):
                answered = self.answer_normal(
                    api_breaker,
                    region,
                    session_token,
                    challenge_id,
                ...
                if "solved" in answered and answered["solved"]:
                    return {
                        ...
                        "bda": {"bda": self.bda, "agent": self.agent},
                    }
        elif challenge["game_data"]["gameType"] == 1:
            for _ in range(waves):
                answered = self.answer_rotate(
